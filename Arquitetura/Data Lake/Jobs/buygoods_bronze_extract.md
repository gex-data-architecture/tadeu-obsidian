...
glue_version: 3.0
ultima_execucao: 2026-06-01 15:44
...
| Timeout (min) | 2880 |
| Max retries | 0 |
| Role | `arn:aws:iam::406933028738:role/buygoods-glue-bronze-role` |
| Script | `s3://api-buygoods-teste-tadeu/glue/scripts/glue_buygoods_bronze.py` |
| Criado | 2026-05-29 09:31:43.878000-03:00 |
| Modificado | 2026-06-01 16:45:15.319000-03:00 |
...
| `--additional-python-modules` | requests,pyarrow,pandas |
...
|---|---|--:|---|
| 2026-06-01 15:44 | STOPPED | 38m30s | — |
| 2026-06-01 15:38 | STOPPED | 4m2s | — |
| 2026-06-01 15:00 | STOPPED | 12m21s | — |
| 2026-06-01 13:54 | FAILED | 49m51s | Command failed with exit code 1 |
| 2026-06-01 13:00 | FAILED | 52m54s | Command failed with exit code 1 |
| 2026-06-01 11:51 | FAILED | 51m40s | Command failed with exit code 1 |
| 2026-06-01 11:00 | FAILED | 50m27s | Command failed with exit code 1 |
| 2026-06-01 09:50 | SUCCEEDED | 31s | — |

## Script

> Fonte: `s3://api-buygoods-teste-tadeu/glue/scripts/glue_buygoods_bronze.py` — baixado do S3 (read-only).
...
Argumentos (todos opcionais, passados como --CHAVE valor):
    ...
    --MAX_WORKERS       (default: 6)    contas em paralelo
    --SLEEP             (default: 0.5)  seg entre requests (por conta)
    --MIN_INTERVAL      (default: 2.0)  seg minimos entre QUAISQUER requests
                        (freio GLOBAL por IP, compartilhado entre os workers;
                        e o que de fato controla a taxa e evita o 429)
    --PAGE_SIZE         (default: 100)
    --ACCOUNT_IDS       (default: vazio = todas)  ex: "12340,12501" para testar
    --FORCE             (default: 0)  1 = ignora checkpoint e reprocessa tudo
    --LOOKBACK_DAYS     (default: 1)  >0 ativa modo INCREMENTAL: re-extrai os
                        ...
                        Default 1 evita que uma run manual sem parametro varra
                        o historico inteiro. Para backfill completo passe
                        explicitamente --LOOKBACK_DAYS 0 (usa START_DATE).
                        ...
                        Valores usados na orquestracao:
                          ...
                          LOOKBACK_DAYS=14  -> 1x por dia na madrugada: janela
                                               D-14. Pega reembolsos/chargebacks
                                               tardios em pedidos recentes (a
                                               grande maioria ocorre ate 14d).
"""
...
MIN_INTERVAL   = float(ARGS.get("MIN_INTERVAL", "0.6"))
...
# curto (5s) -> 10 -> 20 -> 40 -> 80 -> 160 -> teto 180s. O inicio curto
# economiza nos throttles leves; o teto de 180s e necessario porque, sob carga,
# a punicao por IP da BuyGoods persiste por varios minutos e cada retry curto so
# renova a punicao (visto em contas pesadas falhando as 12 tentativas mesmo na
# 2a passada sequencial com cap de 60s). Esperas longas deixam a janela resetar.
# MAX_RETRIES alto garante zero perda de pagina.
BASE_429_WAIT = 5
MAX_429_WAIT  = 180
MAX_RETRIES = 12
REQUEST_TIMEOUT = 30
...
# ----------------------------------------------------------------------
# Limitador de taxa GLOBAL (compartilhado por TODAS as threads/contas)
# ----------------------------------------------------------------------
# O 429 da BuyGoods e por IP: o que importa nao e quantos workers, e o TOTAL
# de requisicoes/seg saindo do job. Reduzir workers sozinho nao basta porque
# varios workers ainda disparam rajadas simultaneas. Este "freio" reserva um
# slot temporal para cada requisicao, garantindo um intervalo minimo
# (MIN_INTERVAL) entre QUAISQUER duas chamadas a API -> elimina as rajadas que
# estouravam o 429 nas contas pesadas. Nao mata a concorrencia: o sleep ocorre
# fora do lock e o GET roda em paralelo; apenas o INICIO de cada request e
# espacado. Throughput maximo global = 1/MIN_INTERVAL req/s.
_rate_lock = threading.Lock()
_next_slot = [0.0]  # holder mutavel (monotonic) do proximo horario liberado
...
def fetch_page(session, account_id, day_str, offset):
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        try:
            rate_limit()  # freio GLOBAL por IP (espaca o inicio de toda request)
...
def main():
    ...
    if errors:
        ...
        log(f"[RETRY] {len(retry_accounts)} conta(s) falharam na 1a passada; "
            f"aguardando 120s p/ resetar a janela de rate-limit e re-tentando "
            f"SEQUENCIALMENTE (freio global continua ativo)...")
        time.sleep(120)
        errors = []
