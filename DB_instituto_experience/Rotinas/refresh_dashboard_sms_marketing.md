...
security: "DEFINER"
criada_em: "2026-05-25 12:16:17"
alterada_em: "2026-05-25 12:16:17"
execucoes: ""
...
- **Escreve:** [[dashboard_sms_marketing]]
- **Cria:** —
- **Trunca:** [[dashboard_sms_marketing]]
- **Dropa:** —
- **Chama:** —
...
```sql
BEGIN
    TRUNCATE TABLE dashboard_sms_marketing;

    INSERT INTO dashboard_sms_marketing (
        data_venda,
        nome_produto,
        niche,
        tipo_venda,
    ...
    WITH
        ...
        raw_sms AS (
            ...
            WHERE cb.created_at_date >= '2025-01-01'
              AND LOWER(cb.offer_name) LIKE '%sms%'
              AND (
            ...
            WHERE bg.created_at_date >= '2025-01-01'
              AND LOWER(bg.offer_name) LIKE '%sms%'
              AND (
        ...
        base_universe AS (
            SELECT DISTINCT created_at_date AS dt, product_key AS pk FROM aggregated_sms
            UNION
